...
const JadwalUjian = () => {
  ...
  const fetchUjianData = () => {
    axios.get('http://194.233.93.124:5005/jadwal-ujian')
      .then(response => {
  ...
  const handleSave = (updatedItem) => {
    ...
    axios.patch(`http://194.233.93.124:5005/jadwal-ujian/${updatedItem.id_ujian}`, updatedData)
      .then(response => {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (confirmUpload) {
      try {
        const response = await axios.post('http://194.233.93.124:5005/jadwal-ujian', formData, {
          headers: {
